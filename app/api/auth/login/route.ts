...
export async function POST(request: Request) {
  ...
  const redirectTo = getAuthRedirectPath(cleanField(formData, "redirect"));
  const config = getSupabasePublicConfig();

  if (!config) {
    return redirectToLogin(request, "config", redirectTo);
  }

  if (!email || !password) {
    return redirectToLogin(request, "missing", redirectTo);
  }
  ...
  if (!response.ok || !result.access_token) {
    ...
    return redirectToLogin(request, "invalid", redirectTo);
  }
