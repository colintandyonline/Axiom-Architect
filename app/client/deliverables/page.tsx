      ...
      <section className="mx-auto max-w-[1440px] border border-[#9ed39f]/24 bg-[#020904] p-6 lg:p-10">
        <div className="grid gap-8 lg:grid-cols-[1fr_360px] lg:items-end">
          <div>
            ...
            <p className="mt-5 max-w-2xl text-lg leading-8 text-white/70">Prepared files and release status for this workspace.</p>
          </div>
          <aside className="border border-[#9ed39f]/35 bg-black p-5">
            ...
            <p className="mt-3 text-sm leading-7 text-white/68">{latest ? tidy(latest.status) : "No files have been released yet."}</p>
          </aside>
        </div>
      </section>
        ...
        <div className="mt-8 grid gap-4">
          {rows.length ? rows.map((row) => (
            <article key={row.id} className="grid gap-4 border border-[#9ed39f]/24 bg-black p-5 lg:grid-cols-[1fr_auto] lg:items-center">
              <div><p className="text-xs font-black uppercase tracking-[0.18em] text-[#9ed39f]">{tidy(row.status)}</p><h3 className="mt-3 break-words text-2xl font-black uppercase tracking-[-0.05em]">{row.title}</h3><p className="mt-2 text-sm leading-7 text-white/68">{row.description || tidy(row.deliverable_type)}</p></div>
            </article>
